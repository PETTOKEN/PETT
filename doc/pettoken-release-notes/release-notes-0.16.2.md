pettoken Core version 0.16.2 is now available from:
...
installer (on Windows) or just copy over `/Applications/pettoken-Qt` (on Mac)
or `pettokend`/`pettoken-qt` (on Linux).
...
### BIP173 (Bech32) Address support ("PETT1..." addresses)
...
pettoken Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
pettoken data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile pettoken Core is now 4.8. No effort will be
...
pettokenproject plans checkpoint distribution.
To deny checkpoint, use the argument `-cmdcheckpoint=false` with pettokend or pettoken-qt or add `cmdcheckpoint=false` to your pettoken.conf. 
It is a function provided for the purpose of invalidating 51% attack on pettoken, but it is not mandatory.
...
* A new field `iswitness` is True for P2WPKH and P2WSH addresses ("PETT1..." addresses), but not for P2SH-wrapped segwit addresses (see below).
...
- pettoken-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- pettoken-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
